...
/// T047g: Verifies Dataverse virtual table access is available per FR-056.
/// </summary>
...
/// T047g: Verifies D365 F&O virtual data entity access is available per FR-056.
/// </summary>
public class D365FoHealthCheck : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            // Attempt a lightweight OData query to verify connectivity
            // $metadata returns 404 with service-to-service auth, so use a real entity with $top=0
            var result = await _d365FoClient.GetAsync<object>(
                "CustomersV3",
...
/// T047g: Verifies blob storage access is available per FR-056.
/// </summary>
