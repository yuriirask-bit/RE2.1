...
|-------------|---------------|---------|
| Dev | `rg-re2-{env}` | Development and integration testing |
| UAT | `rg-re2-{env}` | User acceptance testing |
| Prod | `rg-re2-{env}` | Production |
...
|---------------|-----------|
| `re2-{env}-secrets` | `AzureAdTenantId`, `AzureAdClientId`, `d365foAuthMode`, `d365foClientId`, `d365foClientSecret` |
| `re2-{env}-secrets` | `AzureAdTenantId`, `AzureAdClientId` |
| `re2-{env}-secrets` | `AzureAdTenantId`, `AzureAdClientId` |
...
```bash
# Check API health
curl https://app-re2-api-{env}.azurewebsites.net/health

# Check Web health
curl https://app-re2-web-{env}.azurewebsites.net/health

# Check readiness (external dependencies)
curl https://app-re2-api-{env}.azurewebsites.net/ready
```
...
2. Generate a client secret and store it in the `re2-{env}-secrets` variable group (see section 3.3)
...
1. Open Azure Portal > `func-re2-compliance-{env}` > **Functions**
...
1. Open Azure Portal > `re2-approval-workflow-{env}` > **Overview**
2. Check the Logic App status is **Enabled**
3. The Logic App calls back to `https://app-re2-api-{env}.azurewebsites.net/api/v1/workflows/callback`
...
- API staging: `https://app-re2-api-{env}-staging.azurewebsites.net`
- Web staging: `https://app-re2-web-{env}-staging.azurewebsites.net`

After swap, the production slots serve traffic:
- API: `https://app-re2-api-{env}.azurewebsites.net`
- Web: `https://app-re2-web-{env}.azurewebsites.net`
...
| where cloud_RoleName == "func-re2-compliance-{env}"
...
az webapp deployment slot swap \
  ...
  --name app-re2-api-{env} \
  --slot staging

az webapp deployment slot swap \
  --resource-group rg-re2-{env} \
  --name app-re2-web-{env} \
  --slot staging
```
...
| `Dataverse__Url` | Parameter file | `https://your-org.crm.dynamics.com` |
| `D365FO__ODataEndpoint` | Parameter file | `https://your-d365fo-instance.operations.dynamics.com/data` |
| `D365FO__Resource` | Parameter file | `https://your-d365fo-instance.operations.dynamics.com` |
...
| `BlobStorage__AccountUrl` | Bicep output | `https://stre2{env}.blob.core.windows.net/` |
| `Caching__Enabled` | Bicep | `true` |
| `Caching__RedisConnectionString` | Key Vault reference | `@Microsoft.KeyVault(VaultName=kv-re2-{env};SecretName=RedisConnectionString)` |
